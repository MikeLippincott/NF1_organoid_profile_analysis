...
# python "$git_root"/1.EDA/scripts/0.generate_umap.py
...
for patient in "${patient_array[@]}"; do
    ...
    Rscript "$git_root"/1.EDA/scripts/2.consensus_profiles.r --patient "$patient"
done
...
conda deactivate
conda activate gff_figure_env
Rscript "$git_root"/1.EDA/scripts/6.plot_metrics_and_mAP.r
Rscript "$git_root"/1.EDA/scripts/7.visualize_hits.r
