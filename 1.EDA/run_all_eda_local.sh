...
conda run -n GFF_analysis python "$git_root"/1.EDA/scripts/5a.find_correlation_pairs_for_montages.py
conda run -n gff_figure_env Rscript "$git_root"/1.EDA/scripts/5b.plot_correlation_and_montages.r
...
conda run -n GFF_analysis python "$git_root"/1.EDA/scripts/10.calculate_count_viability_join.py
...
conda run -n GFF_analysis python "$git_root"/1.EDA/scripts/13.calculate_neighbor_features.py
conda run -n gff_figure_env Rscript "$git_root"/1.EDA/scripts/14.plot_neighbor_features.r
conda run -n GFF_analysis python "$git_root"/1.EDA/scripts/15.calculate_intensity_values.py
conda run -n gff_figure_env Rscript "$git_root"/1.EDA/scripts/16.plot_intensity_facets.r
conda run -n GFF_analysis python "$git_root"/1.EDA/scripts/17.calculate_area_volume_by_patient_treatment.py
