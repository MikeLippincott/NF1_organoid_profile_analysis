...
#
# Where does the variance in the linear-model fits go, and why is so much of it residual? The linear models (`0.linear_modeling`, `1.linear_modeling_technical_vars`) attribute ~70-90% of each feature's variance to `residual`. This notebook works only from the saved fit statistics (`residual_pct`, `rsquared`, `term_pct_of_total_var`), so nothing is refit.
#
# Sections:
#
